/*
    ...
    Time: O(n^3) -> O(n^2) states, for each states, determining max coins is
   O(n) Space: O(n^2) -> O(n^2) to store all states
*/
...
TEST(MaxCoins, Example1) {
    Solution sol;
    vector<int> nums = {3, 1, 5, 8};
    int ans = 167;

    ASSERT_EQ(sol.maxCoins(nums), ans);
}

TEST(MaxCoins, Example2) {
    Solution sol;
    vector<int> nums = {1, 5};
    int ans = 10;
