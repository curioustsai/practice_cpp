...
private:
    ...
    int dfs(vector<int>& coins, int amount, int index) {
        ...
        if (mp.find({index, amount}) != mp.end())
            return mp[{index, amount}];
        
        if (coins[index] > amount) {
            mp[{index, amount}] = dfs(coins, amount, index+1);          
        } else {
            mp[{index, amount}] = min(dfs(coins, amount - coins[index], index) + 1, dfs(coins,amount, index+1));
        }
...
private:
  int dfs(vector<int> &coins, int amount, int index) {
    ...
    if (coins[index] > amount) {
      ...
      return min(dfs(coins, amount - coins[index], index) + 1,
